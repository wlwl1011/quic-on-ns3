...
static NodeContainer BuildDumbbellTopo(LinkProperty *topoinfo, int links, int bottleneck_i,
                                       uint32_t buffer_ms, uint32_t delay, TriggerRandomLoss *trigger = nullptr)
{
    int hosts = links + 1;
    NodeContainer topo;
    topo.Create(hosts);
    ...
    for (int i = 0; i < links; i++)
    {
        ...
        std::cout << "QUIC owd " << owd << std::endl;
        std::cout << "QUIC bps " << bps << std::endl;
...
void test_app_on_dumbbell(std::string &instance, float app_start, float app_stop,
                          ...
                          const std::string &cc, int index, uint32_t delay)
{
    ...
    NodeContainer topo = BuildDumbbellTopo(topoinfo_ptr, links, bottleneck_i, buffer_ms, delay, trigger_loss);
    ...
    {
        // install client
        Ptr<QuicClientApp> client_app = CreateObject<QuicClientApp>(type, cc.c_str());
...
int main(int argc, char *argv[])
{
    ...
    std::string tryValue = std::string("0");
    std::string index = std::string("20");
    std::string loss_str("1"); // config random loss
    std::string cc1("cubic");
    std::string cc2("cubic");
    std::string link_delay("100");
    ...
    cmd.AddValue("de", "link_delay", link_delay);

    int try_v = std::stoi(tryValue);
    cmd.Parse(argc, argv);
    int delay = std::stoi(link_delay);
    ...
    std::cout << "loss rate: " << random_loss << std::endl;
    if (loss_integer >= 0)
    {
    ...
    TcpBbrDebug::SetTraceFolder(log_path.c_str());
    TcpTracer::SetTraceFolder(log_path.c_str());
